...
export async function main(ns) {
    ...
    async function checkIfBnIsComplete(ns, player) {
        ...
        // Check if SF3.3 is achieved, corp.js is not already running and we have enough RAM (650GB)
        if (dictOwnedSourceFiles[3] >= 3) {
            try {
                const corpScript = "./corp.js";
                const isCorpRunning = ns.ps("home").some(proc => proc.filename.includes("corp.js"));
                const hasEnoughRam = homeRam >= 700;

                if (!isCorpRunning && ns.fileExists(corpScript)) {
                    if (hasEnoughRam) {
                        log(ns, `Detected SF3.3 achievement with sufficient RAM (${homeRam}GB), executing ${corpScript} script`, true, 'info');
                        launchScriptHelper(ns, corpScript);
                    } else {
                        log(ns, `Detected SF3.3 achievement but insufficient RAM (${homeRam}GB < 650GB) to run ${corpScript}`, true, 'warning');
                    }
                } else if (!isCorpRunning) {
                    log(ns, `Warning: Detected SF3.3 achievement but ${corpScript} script not found`, true, 'warning');
                }
            } catch (err) {
                log(ns, `Error executing corp.js: ${err}`, true, 'error');
            }
        }
